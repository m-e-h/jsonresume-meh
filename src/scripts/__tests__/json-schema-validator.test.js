...
describe("JSONSchemaValidator", () => {
      ...
      ({ data, expectedError }) => {
        ...
        const hasExpectedError = result.errors.some((err) =>
          expectedErrors.some((expected) =>
            err.message.toLowerCase().includes(expected.toLowerCase())
          )
        );
        ...
        if (!hasExpectedError) {
          ...
          console.warn(
            `Expected error containing one of: ${expectedErrors.join(", ")}`
          );
          console.warn(
            `Actual errors: ${result.errors.map((e) => e.message).join("; ")}`
          );
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle malformed data without throwing", () => {
      ...
      expect(() =>
        jsonSchemaValidator.validateResume(malformedData)
      ).not.toThrow();
    });
  });
  ...
  describe("Custom Formats", () => {
    it("should accept flexible date formats", () => {
      ...
      const dateFormatErrors = result.errors.filter(
        (err) => err.keyword === "format" && err.message.includes("date")
      );
